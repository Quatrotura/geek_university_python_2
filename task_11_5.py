...
class Warehouse:
	...
	def __init__(self, name, total_storage_space, occupied_storage_space):
		...
		self.name = name
		self.total_storage_space = total_storage_space 												#cbm
		self.occupied_storage_space = occupied_storage_space										#cbm
		self.free_storage_space = self.total_storage_space - self.occupied_storage_space

	def accept_shipment(self, cargo_qty, cargo_name, volume, shipment_number):
		
		if self.free_storage_space >= volume:
			inventory_name = cargo_name
			self.inventory.setdefault(inventory_name, printer_1.parameters.copy())
			
			if shipment_number == 1:
				del self.inventory[inventory_name]['quantity']
			
			self.inventory[inventory_name].setdefault('accepted_qty')
			
			if self.inventory[inventory_name]['accepted_qty'] == None:
				self.inventory[inventory_name]['accepted_qty'] = 0
			
			self.inventory[inventory_name]['accepted_qty'] += cargo_qty

			self.occupied_storage_space += volume
			print(f'The shipment of {cargo_name} in the amount of {cargo_qty} units has been accepted at warehouse {self.name}.')
			
			return True
		
		else:
			print(f'There is no empty space for storage at this warehouse. We can accept only {self.free_storage_space}.\
 Pls decrease the volume of shipment or submit your request for acceptance after a while.')
			
			return False

	def check_inventory_to_ship(self, cargo_name, cargo_qty):
	
		if cargo_name in self.inventory.keys():
			available_qty = self.inventory[cargo_name]['accepted_qty']
	...
	def ship_to_office(self, cargo_name, cargo_qty, department_name):

		if warehouse_1.check_inventory_to_ship(cargo_name, cargo_qty):
			if cargo_qty > 2:
				print('\nYou can ship max 2 units of one item to one department.')
			
			else:
				self.inventory[cargo_name]['accepted_qty'] -= cargo_qty
				w, l, h = self.inventory[cargo_name]['dimensions'].split('*')
				volume = float(w) * float(l) * float(h) * cargo_qty
				self.occupied_storage_space -= volume
				print(f'\nYour shipping request has been processed and confirmed. {cargo_qty}\
...
class OfficeEquipment:
	...
	def ship_to_warehouse(self, cargo_qty):
		...
		volume = float(w) * float(l) * float(h) * cargo_qty
		response = warehouse_1.accept_shipment(self.cargo_qty, self.cargo_name, volume, self.shipment_number)
...
# проверочка на защиту от дурака (отгрузка со склада несуществующих запасов):

# warehouse_1.ship_to_office('printer Xerox MD-50', 2, 'sales department')
# warehouse_1.ship_to_office('printer Brother BP-34', 2, 'sales department')
# warehouse_1.ship_to_office('scanner Xerox SMD-650', 2, 'sales department')
# warehouse_1.ship_to_office('copying machine Xerox XPMD-200', 2, 'sales department')
...
warehouse_1.ship_to_office('printer Xerox MD-50', 2, 'sales department')
warehouse_1.ship_to_office('printer Brother BP-34', 1, 'procurement department')
warehouse_1.ship_to_office('scanner Xerox SMD-650', 2, 'sales department')
warehouse_1.ship_to_office('copying machine Xerox XPMD-200', 2, 'sales department')
